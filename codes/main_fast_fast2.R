...
for(iter in 1:nsim){
  ...
  for(i in 1:nrow(Y)){
    idx <- which(!is.na(Y[i, ]))
    num <- num + length(idx)
    diff1 <- diff1 + sum(abs(fit_fast3$Xhat[i,idx]-Y[i,idx])^2)
  }
  MISE1_Y <- diff1/num
  # MISE of eigenfucntions
  MISE1_eigen1 <- sum(unlist(lapply(1:K1, function(x){
    min(sum((eigenf_true[[1]][,x]-fit_fast3$efunctions[[1]][,x])^2),
        sum((eigenf_true[[1]][,x]+fit_fast3$efunctions[[1]][,x])^2))})))/(K1*L)
  MISE1_eigen2 <- sum(unlist(lapply(1:K2, function(x){
    min(sum((eigenf_true[[2]][,x]-fit_fast3$efunctions[[2]][,x])^2),
        sum((eigenf_true[[2]][,x]+fit_fast3$efunctions[[2]][,x])^2))})))/(K2*L)
  sim_res[ind,1:8] <- round(c(I, J, L, iter, time_sc[3], MISE1_Y, MISE1_eigen1, MISE1_eigen2),4)
  sim_res[ind,9] <- "mfpca.fast3"
  ind <- ind + 1
