...
#' @param smooth the way of calculating within covariance, "separate" indicating that smooth two parts of within covariance separately and "joint" indicating that smooth within covariance jointly. Defaults to \code{"joint"}.
...
mfpca.fast5 <- function(Y, id, group = NULL, twoway = TRUE, weight = "obs", smooth = "joint",
  ...
  diagD <- rep(Ji, Ji)
  smooth.Gt = face.Cov2(Y=unclass(df$Ytilde), argvals, A0, B, Anew, Bnew, G_invhalf, s)
  ...
  if(weight=="subj"){
    ...
    smooth.Gt <- face.Cov2(Y=YH, argvals, A0, B, Anew, Bnew, G_invhalf, s)
    rm(YH)
  }
  ...
  if(smooth=="joint"){
    ...
    smooth.Gw1 <- face.Cov2(Y = YD, argvals, A0, B, Anew, Bnew, G_invhalf, s)
    ### second part of formula: t(E %*% Y) %*% E %*% Y
    inx_row_ls <- split(1:nrow(df$Ytilde), f=factor(df$id, levels=unique(df$id)))
    YE <- t(vapply(inx_row_ls, function(x) colSums(df$Ytilde[x,,drop=FALSE],na.rm=TRUE), numeric(L)))
    smooth.Gw2 <- face.Cov2(Y = YE, argvals, A0, B, Anew, Bnew, G_invhalf, s)
    # Obtain Kw
    ...
    smooth.Gw <- list(decom=temp, evalues=Sigma[1:N], efunctions=Bnew%*%Eigen$vectors[,1:N])
    rm(Ji, diagD, inx_row_ls, YD, YE, smooth.Gw1, smooth.Gw2, temp, Eigen, Sigma, d, per, N, B, Anew, G_invhalf, s)
  }
